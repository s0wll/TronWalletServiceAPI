...
class WalletInfoCRUD:
    ...
    async def create_wallet_info(self, wallet_info: WalletResponse) -> Wallet:
        create_stmt = (
            insert(WalletInfoORM).values(**wallet_info.model_dump()).returning(WalletInfoORM)
        )
    ...
    async def get_wallet_info_list(self, limit: int, offset: int) -> list[Wallet]:
        ...
        models = [
            Wallet.model_validate(one, from_attributes=True) for one in result.scalars().all()
        ]
        if not models:
